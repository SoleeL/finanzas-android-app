...
fun AccountSelectionScreen(
    ...
    Column(
        ...
        Button(onClick = onBackPress) {
            Text(text = "Back to Account Type Selection")
        }

        Spacer(modifier = Modifier.height(8.dp))

        Button(onClick = onContinue) {
            Text(text = "Continue to Instalment Selection")
        }
    }
}
...
private fun ExpenseSummaryHeader(createExpenseViewModel: CreateExpenseViewModel) {
    ...
    // <sentencia previa> que pague con crédito.
    // <sentencia previa> que pague con débito.
    // <sentencia previa> que pague con efectivo.
